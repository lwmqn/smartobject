...
const ObjInstance = require('./object_instance')
...
function SmartObject (hal, setup) {
  ...
  hal = hal || {}
  Object.defineProperty(this, 'hal', {
    value: hal, writable: true, enumerable: false, configurable: false
  })
  Object.defineProperty(this, 'ipsoOnly', {
...
SmartObject.prototype.init = function (oid, iid, resrcs, setup) {
  const objInst = this.findObjectInstance(oid, iid) || this.create(oid, iid)
...
SmartObject.prototype.create = function (oid, iid) {
  if (!utils.isValidArgType(iid)) throw new TypeError('iid should be given with a number or a string.')

  const oidKey = utils.getOidKey(oid)
  this[oidKey] = this[oidKey] || {}

  if (!_.isNil(this[oidKey][iid])) throw new Error(`Object instance of iid ${iid} already exists. Cannot create.`)

  if (this.ipsoOnly) {
    if (!_.isNumber(iid)) throw new TypeError('iid must be a number (ipsoOnly).')
    else if (!utils.isIpsoOid(oid)) throw new Error('oid not in an IPSO-defined Object.')
  }

  const objInst = this[oidKey][iid] = new ObjInstance(oidKey, iid, this)
...
SmartObject.prototype.objectList = function () {
  ...
  _.forEach(this, (obj, oidKey) => {
    const iidList = []

    if (!_.isObject(obj)) return

    _.forEach(obj, (inst, iid) => {
      let iidNum = parseInt(iid)
      iidNum = _.isNaN(iidNum) ? iid : iidNum

      if (_.isObject(inst)) iidList.push(iidNum)
    })
...
SmartObject.prototype.has = function (oid, iid, rid) {
  const oidKey = utils.getOidKey(oid)
  const obj = this[oidKey]
  let has = !!obj
  let objInst

  if (!_.isUndefined(iid) && !utils.isValidArgType(iid)) throw new TypeError('iid should be given with a number or a string.')
...
SmartObject.prototype.findObjectInstance = function (oid, iid) {
  ...
  const target = this.findObject(oid)
  return target ? target[iid] : target
}

SmartObject.prototype.isReadable = function (oid, iid, rid) {
  const rsc = this.get(oid, iid, rid)
  let readable = false
  let rdef

  if (_.isUndefined(rsc)) return readable
...
SmartObject.prototype.isWritable = function (oid, iid, rid) {
  ...
  if (_.isUndefined(rsc)) return writable
...
SmartObject.prototype.isExecutable = function (oid, iid, rid) {
  ...
  if (_.isUndefined(rsc)) return executable
...
SmartObject.prototype.get = function (oid, iid, rid) {
  const objInst = this.findObjectInstance(oid, iid)
...
SmartObject.prototype.set = function (oid, iid, rid, value) {
  ...
  if (objInst) objInst.set(rid, value)

  return set
}

SmartObject.prototype.dumpSync = function (oid, iid) {
  const dumpType = ['so', 'obj', 'objInst'][arguments.length]
  let target

  if (!dumpType) throw new Error('Bad arguments. What do you like to dump?')
...
SmartObject.prototype.dump = function (oid, iid, opt, callback) {
  ...
  if (!_.isFunction(callback)) throw new TypeError('Callback should be a function.')

  if (dumpType === 'objInst') {
    target = this.findObjectInstance(oid, iid)
    if (target) target.dump(opt, callback)
  } else if (dumpType === 'obj') {
    target = this.findObject(oid)
    if (target) utils.dumpObject(target, opt, callback)
  } else if (dumpType === 'so') {
    target = this
    utils.dumpSmartObject(target, opt, callback)
  }

  if (!target) utils.invokeCbNextTick(new Error('Target not found. Cannot dump.'), null, callback)
