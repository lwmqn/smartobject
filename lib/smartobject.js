...
SmartObject.prototype.dump = function (oid, iid, callback, opt) {
    ...
    if (arguments.length === 1) {                               // (callback)
        ...
        if (_.isFunction(oid) && _.isPlainObject(iid)) {        // (callback, opt)
            opt = iid;
            callback = oid;
            dumpType = 'so';
        } else {                                                // (oid, callback)
        ...
        if (_.isFunction(iid) && _.isPlainObject(callback)) {   // (oid, callback, opt)
            opt = callback;
            callback = iid;
            dumpType = 'obj';
    ...
    } else if (arguments.length === 4) {                        // (oid, iid, callback, opt)
        dumpType = 'objInst';
    } else {
...
SmartObject.prototype.read = function (oid, iid, rid, callback, opt) {
    var objInst = this.findObjectInstance(oid, iid);
    return objInst ? objInst.read(rid, callback, opt) : utils.invokeCbNextTick(new Error('Object or Object Instance not found.'), '_notfound_', callback);
};

SmartObject.prototype.write = function (oid, iid, rid, value, callback, opt) {
    var objInst = this.findObjectInstance(oid, iid);
    return objInst ? objInst.write(rid, value, callback, opt) : utils.invokeCbNextTick(new Error('Object or Object Instance not found.'), '_notfound_', callback);
};
