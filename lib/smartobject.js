...
/*************************************************************************************************/
SmartObject.prototype.init = function (oid, iid, resrcs, _state) {
    var objInst;

    objInst = this.findObjectInstance(oid, iid) || this.create(oid, iid, _state);

    // init() will auto create namespace, and will firstly clear all resources in the Object Instance
    objInst.init(resrcs, _state);
    return objInst;
};

SmartObject.prototype.create = function (oid, iid, _state) {
    var oidKey,
        objInst;
    ...
    objInst = this[oidKey][iid] = new ObjInstance(oidKey, iid, this);   // default _state = { ipsoOnly: parent.ipsoOnly }

    if (_.isObject(_state))
        _.assign(objInst._state, _state);

    if (objInst._state.ipsoOnly) {
        if (!_.isNumber(iid))
