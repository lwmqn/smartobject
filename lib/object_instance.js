...
function ObjectInstance(oidKey, iid, parent) {
    ...
    Object.defineProperty(this, '_state', { value: { ipsoOnly: parent.ipsoOnly }, writable: false, enumerable: false, configurable: false });
}

/*************************************************************************************************/
/*** Public Methods: Getter and Setter                                                         ***/
/*************************************************************************************************/
ObjectInstance.prototype.init = function (resrcs, _state) {
    // each time of init(), all resources and inner this._state will be cleared. Use .set() to add/modify resource
    var self = this.clear(),    // this._state default to { ipsoOnly: this.parent.ipsoOnly }
        ridKey;

    if (!_.isPlainObject(resrcs))
        throw new TypeError('Resources should be wrapped in an object.');
    
    if (!_.isUndefined(_state) && !_.isPlainObject(_state))
        throw new TypeError('_state should be an object if given.');
    else
        _.assign(this._state, _state);

    _.forEach(resrcs, function (rVal, rKey) {
        if (self._state.ipsoOnly) {
...
ObjectInstance.prototype.clear = function () {
    ...
    this._state.ipsoOnly = this.parent.ipsoOnly;
